...
const techIconMap = {
  ...
  postgresql: { class: "fa-database", prefix: "fas" }, 
  flask: { class: "fa-flask", prefix: "fas" }, 
};
...
export default function ProjectModal({ project, onClose }) {
  ...
  {project.tech.map((tech, idx) => {
    ...
    return icon ? (
      <i key={idx} className={`${icon.prefix} ${icon.class}`} title={tech}></i>
    ) : (
      <span key={idx}>{tech}</span> // fallback text if no icon found
    );
  })}
</div>
